...
public sealed class TableStorage
{
    ...
    {
        if (!type.IsUnmanagedType) throw new ArgumentException($"Type {type} is not a unmanaged type.", nameof(type));
        if (Storages.TryGetValue(type, out var array)) return array;
        throw new ArgumentException($"invalid StorageType: {type.Type}", nameof(type));
    }
